# Hi, I'm Mustafa Mohamed 👋

**Cybersecurity Analyst | Penetration Tester | Red Team | Web Application Security**

I'm a cybersecurity professional with hands-on experience in penetration testing, Active Directory security assessments, and infrastructure administration.

I enjoy building offensive security tools, discovering vulnerabilities, and automating security workflows using Python.

## About Me

- Cybersecurity Analyst with hands-on penetration testing experience
- Web Application Security Testing (OWASP Top 10)
- Active Directory Security Assessments
- Internal Network Penetration Testing
- Windows & Linux Administration
- Python Automation for Security
- VMware & FortiGate Administration

## Technical Skills

### Offensive Security

- Penetration Testing
- Red Team Operations
- Active Directory Exploitation
- Web Application Security
- Vulnerability Assessment

### Programming

- Python
- Bash
- PowerShell

### Security Tools

- Burp Suite
- Metasploit
- BloodHound
- Impacket
- Nmap
- SQLMap
- Nessus
- Hashcat
- Hydra
- Wireshark

## Featured Projects

| Project | Description |
|---------|-------------|
| Web Security Toolkit | Python tools for web security testing |
| Active Directory Labs | AD enumeration and attack techniques |
| Network Assessment Scripts | Automation scripts for reconnaissance |
| Security Reports | Sample penetration testing reports |

## Training & Certifications

- CEH – Certified Ethical Hacker
- eJPT – eLearnSecurity Junior Penetration Tester
- OSCP & OSCP+ (Self-Study Track) – Offensive Security Certified Professional Techniques
- eWPT – eLearnSecurity Web Penetration Tester
- eWPTX – Web Application Penetration Tester eXtreme
- CRTP – Certified Red Team Professional
- Python for Cybersecurity & Offensive Automation
- Additional Technical Training: 
  - Microsoft Azure Security Basics 
  - FortiGate Firewall Essentials 
  - VMware vSphere Fundamentals 
  - CCNA, CCNP ENCOR Concepts, MCSA Fundamentals 

## Labs

- TryHackMe
- Hack The Box
- PortSwigger Web Security Academy

## Connect with Me

- LinkedIn: https://www.linkedin.com/in/mustafa-mohamed-0a77a1197/
- Email: m.bafy10@gmail.com
